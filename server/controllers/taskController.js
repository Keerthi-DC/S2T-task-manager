...
// Fetch tasks directly from the TeacherTask model
const fetchTeacherTasks = async (classId) => {
  try {
    const tasks = await TeacherTask.find({ className: classId });  // Fetch tasks for the given class
    
    if (tasks.length === 0) {
      return { message: `No tasks found for class ${classId}.` };  // Return a message if no tasks are found
    }
    
    return tasks;  // If tasks are found, return them
  } catch (error) {
